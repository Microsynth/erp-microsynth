...
def populate_eur_fields_on_customs_declarations(to_date='2025-07-06'):
    ...
    try:
        for i, cd in enumerate(customs_declarations):
            ...
            cd_doc.flags.ignore_links = True  # allow to save even if cancelled or disabled documents are linked
            cd_doc.save()
...
def migrate_debtors(customer_id, company, currency, verbose=True):
    ...
    try:
        ...
        comment = frappe.get_doc({
            ...
            'content': "Inactivated due change of debtor accounts.",  # f"Customer renamed from {customer_id} to {renamed_customer_id} as part of debtor migration to create a new customer {customer_id} with billing currency {currency}.",
...
def get_total_outstanding_credits(credit_data):
    """
    Calculate the total outstanding credits from the given credit_data list.
    """
    total = 0
...
def create_legacy_credit_account(customer_id, company, credit_type, credit_data, verbose_level=0, contact_id=None, dry_run=False, contact_replacement_map={}):
    ...
    if get_total_outstanding_credits(credit_data) <= 0.01:
        if verbose_level > 0:
            log("WARNING", "No outstanding credits. Skipping creation.")
        return
    ...
    if credit_type == "Project":
        credit_accounts = frappe.db.sql("""
            SELECT DISTINCT `tabProduct Type Link`.`parent` AS `name`
            FROM `tabProduct Type Link`
            INNER JOIN `tabCredit Account`
                ON `tabCredit Account`.`name` = `tabProduct Type Link`.`parent`
            WHERE
                `tabProduct Type Link`.`parenttype` = 'Credit Account'
                AND `tabProduct Type Link`.`parentfield` = 'product_types'
                AND `tabProduct Type Link`.`product_type` = %s
                AND `tabCredit Account`.`customer` = %s
    ...
    for credit in credit_data:
        ...
        if credit_type == "Project" and credit.get('product_type') != "Project":
            if verbose_level > 0:
                log("INFO", f"Skipping credit {credit.get('sales_invoice')} - not Project type.")
            continue

        si_doc = frappe.get_doc("Sales Invoice", credit.get('sales_invoice'))

        if si_doc.docstatus != 1:
            log("ERROR", f"Deposit invoice {si_doc.name} not submitted (docstatus={si_doc.docstatus}).")
            error = True
            continue

        if si_doc.company != company:
            log("ERROR", f"Invoice company mismatch: {si_doc.company}.")
            error = True
            continue

        if frappe.get_value("Customer", si_doc.customer, "disabled"):
            log("ERROR", f"Customer '{si_doc.customer}' is disabled.")
            error = True
            continue

        if si_doc.customer != customer_id:
            log("ERROR", f"Customer mismatch: invoice={si_doc.customer}.")
            error = True
            continue

        if not contact_id:
            if not si_doc.contact_person:
                log("ERROR", f"Deposit invoice {si_doc.name} has no Contact Person.")
                error = True
                continue
        ...
        credit_account_doc = frappe.get_doc({
            ...
            "contact_person": contact_id or si_doc.contact_person,
            "account_type": "Legacy",
        ...
        if not dry_run:
            ...
            if verbose_level > 0:
                log("INFO", f"Created Credit Account {credit_account_doc.name} based on deposit invoice {si_doc.name}.")
        else:
            if verbose_level > 0:
                log("DRY-RUN", f"Would insert Credit Account for Sales Invoice {si_doc.name} based on deposit invoice {si_doc.name}.")
        break
    ...
    if credit_account_doc:
        for credit in credit_data:
            ...
            if si_doc.credit_account == credit_account_doc.name:
                # will be printed if a Sales Invoice was deducted from more than one deposit invoice
                log("WARNING", f"Sales Inovice {si_name} is already linked to Credit Account {credit_account_doc.name}, skipping.")
                continue

            if si_doc.credit_account and si_doc.credit_account != credit_account_doc.name:
                log("ERROR", f"Sales Invoice {si_name} is already linked to another Credit Account ({si_doc.credit_account}), not going to link to Credit Account {credit_account_doc.name}.")
                continue

            if si_doc.docstatus != 1:
                log("ERROR", f"Sales Invoice {si_name} is not submitted (docstatus={si_doc.docstatus}), skipping.")
                continue
...
def link_legacy_credit_accounts_to_sales_orders(verbose=False):
    """
    Migration function:
    ...
    if not legacy_credit_accounts:
        print("WARNING: No Legacy Credit Accounts found. Nothing to link, going to return.")
        return
    ...
    if not open_sales_orders:
        print("WARNING: No open Sales Orders without Credit Accounts found. Going to return.")
        return
    ...
    for so in open_sales_orders:
        ...
        if not candidate_accounts:
            if verbose:
                print(f"INFO: No Legacy Credit Account found for open Sales Order {so['name']} (Customer {so['customer']}, Company {so['company']}). Skipping.")
            continue
        ...
        if not eligible:
            print(f"INFO: No suitable Legacy Credit Account found for open Sales Order {so['name']} (product_type={so['product_type']}). {candidate_accounts=}. Skipping.")
            continue

        eligible_set = set(acc["credit_account"] for acc in eligible)
        if len(eligible_set) > 1:
            print(f"WARNING: Multiple eligible Legacy Credit Accounts found for Sales Order {so['name']} with Product Type {so['product_type']}. {eligible_set=}")
        ...
        try:
            ...
            linked_count += 1
            print(f"INFO: Linked Legacy Credit Account(s) {eligible_set} on open Sales Order {so['name']}.")
        except Exception as err:
            #frappe.log_error(f"Error linking {credit_account} to {so['name']}: {err}", "link_legacy_credit_accounts_to_sales_orders")
            print(f"WARNING: Failed linking {credit_account} to open Sales Order {so['name']}, going to force it: {err}")

            # FORCE INSERT child rows without triggering validation
            for credit_account in eligible_set:
                try:
                    frappe.db.sql("""
                        INSERT INTO `tabCredit Account Link`
                            (`name`,
                             `parent`,
                             `parenttype`,
                             `parentfield`,
                    ...
                    """, (
                        ...
                        1                            # idx (safe default, ERP will reorder on next save)
                    ))
                    frappe.db.commit()

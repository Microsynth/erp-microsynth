...
def get_columns(filters):
    return [
        ...
        {"label": _("Created on"), "fieldname": "creation", "fieldtype": "Date", "width": 125 },
    ]
...
def get_data(filters):
    ...
    if filters.get('from_barcode') and filters.get('to_barcode'):
        ...
        if from_barcode.isnumeric() and to_barcode.isnumeric():
            ...
            to_prefix = ''.join([i for i in to_barcode if not i.isdigit()])
            if from_prefix != to_prefix:
                frappe.throw("From Barcode and To Barcode need to have the same Prefix.")
            from_barcode = re.sub("[^0-9]", "", from_barcode)
            to_barcode = re.sub("[^0-9]", "", to_barcode)
            if len(from_barcode) != len(to_barcode):

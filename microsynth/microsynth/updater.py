...
def disable_notifications():
    ...
    for notification in notifications:
        if notification['name'] not in ['Retention Bonus', 'Notification for new fiscal year', 'Training Scheduled', 'Training Feedback', 'Material Request Receipt Notification', 'Exit Interview Scheduled']:
...
def disable_hot_config_in_dev():
    ...
    if "erp.microsynth.local" not in (frappe.conf.host_name or ""):
        ...
        config.certificate_import_path = (config.certificate_import_path or "").replace("/erp_share/", "/erp_share_test/")
        config.website_applications_path = None

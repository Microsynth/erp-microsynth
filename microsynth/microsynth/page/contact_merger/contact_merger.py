...
def merge_contacts(contact_1, contact_2, values):
    ...
    try:
        ...
        address_type_1 = frappe.get_value("Address", frappe.get_value("Contact", contact_1, "address"), "address_type")
        address_type_2 = frappe.get_value("Address", frappe.get_value("Contact", contact_2, "address"), "address_type")
        ...
        # remove lock
        frappe.db.sql(f"""DELETE FROM `tabContact Lock` WHERE `tabContact Lock`.`contact` = '{contact_2}'; """)
        frappe.db.commit()
